...
async function evaluateCard(
  ...
  const promptIntro = isRussian
    ? `Ты команда опытных стартап-экспертов, оценивающих карточку "${cardDef.id}".
Оценивай объективно и справедливо, как бы ты оценивал любую другую работу.`
    : `You are a team of experienced startup experts evaluating a "${cardDef.id}" card.
Evaluate objectively and fairly, as you would evaluate any other work.`;
...
- 8-10: Отлично - конкретный, глубокий контент с инсайтами
- 6-7: Хорошо - есть суть, можно улучшить
- 4-5: Базово - общие фразы, нужна доработка
- 1-3: Слабо - требует переработки
Оценивай объективно, признавай сильные стороны.`
    : `SCORING GUIDELINES:
- 8-10: Excellent - specific, deep content with insights
- 6-7: Good - has substance, room to improve
- 4-5: Basic - generic phrases, needs work
- 1-3: Weak - needs rework
Evaluate objectively, acknowledge strengths.`;

  const evaluateLabel = isRussian ? 'Оцени по 7 критериям' : 'Evaluate on these 7 criteria';
  const instructionLabel = isRussian
    ? 'Для каждого критерия укажи оценку (1-10) и конструктивное объяснение (1-2 предложения).'
    : 'For each criterion, provide a score (1-10) and a constructive explanation (1-2 sentences).';
