...
export const GroupChatDrawer = ({
  ...
  const currentResponderCharacter = currentResponder ? getCharacterById(currentResponder, language) : null;
  const allCharacters = getAllCharacters(language);
  const availableToAdd = allCharacters.filter(
            ...
            {selectedCharacters.map((charId) => {
                ...
                return (
                  <div key={charId} className="relative group">
                    <Avatar 
                      className="h-10 w-10 transition-transform group-hover:scale-105 cursor-pointer"
                      style={{ boxShadow: `0 0 0 2px ${character.color}` }}
                      onClick={() => setDetailCharacterId(charId)}
                    >
            ...
            <div className="space-y-4">
              {messages.map((message) => (
                <GroupChatMessage
                  ...
                  onAvatarClick={setDetailCharacterId}
                />
              ))}
